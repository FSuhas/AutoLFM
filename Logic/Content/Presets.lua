...
local function captureCurrentState()
  ...
  if AutoLFM.Core.Persistent and AutoLFM.Core.Persistent.DeepCopy then
    state.dungeonNames = AutoLFM.Core.Persistent.DeepCopy(state.dungeonNames)
    state.roles = AutoLFM.Core.Persistent.DeepCopy(state.roles)
    state.activeChannels = AutoLFM.Core.Persistent.DeepCopy(state.activeChannels)
  end
...
AutoLFM.Core.SafeRegisterInit("Logic.Content.Presets", function()
  ...
  id = "I17",
  dependencies = { "Core.Persistent", "Logic.Selection" }
})
