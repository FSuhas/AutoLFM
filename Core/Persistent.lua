...
for i = 1, table.getn(SETTINGS_REGISTRY) do
  ...
  AutoLFM.Core.Persistent["Get" .. capitalizedKey] = function()
      return AutoLFM.Core.Persistent.Get(setting.key, setting.default)
  end
  AutoLFM.Core.Persistent["Set" .. capitalizedKey] = function(value)
      local coerced = value
      ...
      AutoLFM.Core.Persistent.Set(setting.key, coerced)
  end
end
...
function AutoLFM.Core.Persistent.SetDungeonFilter(filterId, enabled)
  ...
  AutoLFM.Core.Persistent.SetDungeonFilters(filters)
end
...
AutoLFM.Core.Persistent.DeepCopy = deepCopy
...
function AutoLFM.Core.Persistent.SetBroadcastInterval(interval)
  -- Clamp between 30 seconds and 2 hours
  local clamped = math.max(30, math.min(7200, tonumber(interval) or 60))
  AutoLFM.Core.Persistent.Set("broadcastInterval", clamped)
end
...
function AutoLFM.Core.Persistent.Init()
  ...
  if charData and charData.isHardcore == nil then
    ...
    AutoLFM.Core.Persistent.SetIsHardcore(isHardcore)
  end
end
...
-- Registration
-----------------------------------------------------------------------------
AutoLFM.Core.SafeRegisterInit("Core.Persistent", function()
