...
function AutoLFM.UI.Content.Presets.OnLoad(frame)
  ...
  isCondensed = AutoLFM.Core.Persistent.GetPresetsCondensed()
end
...
local function createPresetRow(index, presetName, presetData, isFirst, isLast, yOffset)
  ...
  if not isFirst then
    ...
    upBtn:SetScript("OnClick", function()
      if AutoLFM.Core.Persistent.MovePresetUp(presetName) then
        AutoLFM.UI.Content.Presets.Refresh()
      end
    end)
  end
  ...
  if not isLast then
    ...
    downBtn:SetScript("OnClick", function()
      if AutoLFM.Core.Persistent.MovePresetDown(presetName) then
        AutoLFM.UI.Content.Presets.Refresh()
      end
    end)
  end
