--=============================================================================
-- AutoLFM: Dungeons Panel (Compact)
--=============================================================================
...
local function EachColor(cb)
    local colors = AutoLFM.Core.Constants.PRIORITY_COLORS
    if colors then for i, c in ipairs(colors) do if c then cb(c, i) end end end
end

local function RefreshDisplay() if AutoLFM.UI.DungeonsPanel.Refresh then AutoLFM.UI.DungeonsPanel.Refresh() end end
...
function AutoLFM.UI.DungeonsPanel.GetFilterState(key) return key and filterStates[key] or false end
...
local function UpdateFilterLabelColor()
    if filterLabelText then AutoLFM.Core.Utils.SetFontColor(filterLabelText, HasDisabledFilter() and "gold" or "white") end
end
...
local function CreateFilterCheckbox(parent, color, xOffset)
    ...
    for _, tex in pairs({cb:GetNormalTexture(), cb:GetCheckedTexture(), cb:GetDisabledCheckedTexture()}) do
...
local function CreateColorFilterUI(parent)
    ...
    filterLabelFrame:SetScript("OnLeave", function() UpdateFilterLabelColor() GameTooltip:Hide() end)
...
function AutoLFM.UI.DungeonsPanel.UpdateFilterUI()
    for k, cb in pairs(filterCheckboxes) do if cb and cb.SetChecked then cb:SetChecked(filterStates[k]) end end
    RefreshDisplay()
end
...
local function UpdateRowVisibility()
    local sorted = AutoLFM.Logic.Content.GetSortedDungeons and AutoLFM.Logic.Content.GetSortedDungeons(UnitLevel("player") or 1)
    ...
    for _, f in pairs(dungeonRows) do if f then AutoLFM.UI.PanelBuilder.UpdateScrollHeight(f:GetParent(), visible) break end end
end
...
function AutoLFM.UI.DungeonsPanel.Display(parent)
    ...
    local sorted = AutoLFM.Logic.Content.GetSortedDungeons(UnitLevel("player") or 1) or {}
    local yOffset, visible = 0, 0
    for _, e in ipairs(sorted) do
...
function AutoLFM.UI.DungeonsPanel.Init()
    ...
    filterFrame = CreateColorFilterUI(p.bottomZone) filterFrame:Show()
